...
const UserReview = ({ id, contentType, notifyError }) => {
  ...
  const isContentLoaded = Object.keys(userReview).length > 0 && userReview.total_results > 0;
  ...
  const imageURL = isContentLoaded && `${REVIEWER_PROFILE_BASE_URL}${userReview.results[0].author_details.avatar_path}`;
  const rating = isContentLoaded && userReview.results[0].author_details.rating * 10;
  const { author } = isContentLoaded && userReview.results[0];
  const createdAt = isContentLoaded && formatDate(userReview.results[0].created_at);
  const review = isContentLoaded && userReview.results[0].content;
